...
uses
  ...
  StdCtrls, ojVirtualTrees, ExtDlgs, ImgList, Buttons, ExtCtrls, ComCtrls,
  Mask;
...
type
  ...
  private
    ...
    FTree: TojVirtualStringTree; // A back reference to the tree calling.
    FNode: TojVirtualNode;       // The node being edited.
...
begin
  ...
  Data := FNode.UserData as TojPropertyData;
  if FEdit is TComboBox then
...
function TPropertyEditLink.PrepareEdit(Tree: TojBaseVirtualTree; Node: TojVirtualNode; Column: TColumnIndex): Boolean;
...
begin
  Result := True;
  FTree := Tree as TojVirtualStringTree;
  FNode := Node;
  FColumn := Column;
...
begin
  ...
  Data:= FNode.UserData as TGridData;
  if FEdit is TComboBox then
  begin
    S := TComboBox(FEdit).Text;
...
function TGridEditLink.PrepareEdit(Tree: TojBaseVirtualTree; Node: TojVirtualNode; Column: TColumnIndex): Boolean;
...
begin
  ...
  Data:= Node.UserData as TGridData;
