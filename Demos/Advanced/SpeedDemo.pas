...
uses
  ...
  StdCtrls, ojVirtualTrees, ExtDlgs, ComCtrls, jpeg, Menus;
...
type
  TSpeedForm = class(TForm)
    ...
    procedure VST1GetText(Sender: TojBaseVirtualTree; Node: TojVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
    procedure VST1Change(Sender: TojBaseVirtualTree; Node: TojVirtualNode);
    procedure VST1StructureChange(Sender: TojBaseVirtualTree; Node: TojVirtualNode; Reason: TChangeReason);
    procedure VST1StateChange(Sender: TojBaseVirtualTree; Enter, Leave: TVirtualTreeStates);
  end;
...
procedure TSpeedForm.VST1Change(Sender: TojBaseVirtualTree; Node: TojVirtualNode);
begin
  Label6.Caption := Format('Selected: %d', [VST1.SelectedCount]);
end;

procedure TSpeedForm.VST1GetText(Sender: TojBaseVirtualTree; Node: TojVirtualNode;
  Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
begin
  CellText := Format('Level %d, Index %d', [Sender.GetNodeLevel(Node), Node.Index]);
end;

procedure TSpeedForm.VST1StateChange(Sender: TojBaseVirtualTree; Enter, Leave: TVirtualTreeStates);
begin
  if not (csDestroying in ComponentState) then
    UpdateStateDisplay(Sender.TreeStates, Enter, Leave);
end;

procedure TSpeedForm.VST1StructureChange(Sender: TojBaseVirtualTree;
  Node: TojVirtualNode; Reason: TChangeReason);
begin
  Label6.Caption := Format('Selected: %d', [VST1.SelectedCount]);
end;
