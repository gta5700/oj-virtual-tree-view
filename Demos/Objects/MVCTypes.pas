...
uses Windows,Messages,SysUtils,Graphics,ojVirtualTrees,Classes,StdCtrls,
...
type { TMVCNode is the encapsulation of a single Node in the structure.
     ...
     TMVCNode=class(TObject)
       ...
       public
         ...
         property VirtualNode:TojVirtualNode read FVirtualNode write FVirtualNode;
       end;
     ...
     TMVCEditLink=class;
      ...
      TMVCTreeViewNode = class(TojVirtualNode)
      private
        //  instead of using FInternalDataOffset
        MyNode: TMVCNode;
      public
      end;

     TMVCTreeView=class(TojBaseVirtualTree)
       private
         { This is a pointer to the structure associated with
           this viewer. }
         FTree:TMVCTree;
         // GTA no internal data but TMVCTreeViewNode
         ...
         function GetMVCNode(VirtualNode:TojVirtualNode):TMVCNode;
         procedure SetMVCNode(VirtualNode:TojVirtualNode; aNode:TMVCNode);
       ...
       protected
         ...
         function DoGetNodeWidth(Node: TojVirtualNode; Column: TColumnIndex; Canvas: TCanvas = nil): Integer; override;
         procedure DoPaintNode(var PaintInfo: TVTPaintInfo); override;
         function DoInitChildren(Node: TojVirtualNode; var ChildCount: Cardinal): Boolean; override;
         procedure DoInitNode(aParent,aNode:TojVirtualNode;
                              var aInitStates:TVirtualNodeInitStates); override;
         procedure DoFreeNode(aNode:TojVirtualNode); override;
         function DoGetImageIndex(Node: TojVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
           var Ghosted: Boolean; var Index: TImageIndex): TCustomImageList; override;
         procedure DoChecked(aNode:TojVirtualNode); override;
         function DoCreateEditor(Node: TojVirtualNode; Column: TColumnIndex): IVTEditLink; override;
         // function InternalData(Node: TojVirtualNode): Pointer;
         function InternalData(Node: TojVirtualNode): TMVCTreeViewNode;
         // GTA useless
         // function InternalDataSize: Cardinal;

         function GetOptionsClass: TTreeOptionsClass; override;
       public
          function getDefaultNodeClass: TojVirtualNodeClass;override;
         ...
         property MVCNode[VirtualNode:TojVirtualNode]:TMVCNode read GetMVCNode;
   ...
   TMVCEditLink=class(TInterfacedObject,IVTEditLink)
                   private
                     ...
                     FNode:TojVirtualNode;               // the node to be edited
                   ...
                   public
                     ...
                     function PrepareEdit(Tree: TojBaseVirtualTree; Node: TojVirtualNode; Column: TColumnIndex): Boolean; stdcall;
...
function TMVCTreeView.GetMVCNode(VirtualNode:TojVirtualNode):TMVCNode;
begin
  ...
  if VirtualNode.IsAssigned() then
    ...
    Result := InternalData(VirtualNode).MyNode
  else
    Result := nil;
end;

procedure TMVCTreeView.SetMVCNode(VirtualNode:TojVirtualNode;aNode:TMVCNode);
begin
  { Note the relationship between a VirtualNode and the TMVCNode it
    represents in the Nodes data. }
  InternalData(VirtualNode).MyNode:= aNode;
  //  PMyNodeData(InternalData(VirtualNode)).Node:=aNode;
end;

function TMVCTreeView.DoCreateEditor(Node: TojVirtualNode; Column: TColumnIndex): IVTEditLink;
...
function TMVCTreeView.DoGetNodeWidth(Node: TojVirtualNode; Column: TColumnIndex; Canvas: TCanvas = nil): Integer;
...
function TMVCTreeView.getDefaultNodeClass: TojVirtualNodeClass;
begin
  result:= TMVCTreeViewNode;
end;

procedure TMVCTreeView.DoFreeNode(aNode:TojVirtualNode);
...
function TMVCTreeView.DoInitChildren(Node: TojVirtualNode; var ChildCount: Cardinal): Boolean;
begin
...
procedure TMVCTreeView.DoInitNode(aParent,aNode:TojVirtualNode;
...
function TMVCTreeView.DoGetImageIndex(Node: TojVirtualNode; Kind: TVTImageKind; Column: TColumnIndex;
...
procedure TMVCTreeView.DoChecked(aNode:TojVirtualNode);
...
function TMVCTreeView.InternalData(Node: TojVirtualNode): TMVCTreeViewNode;
...
begin
  if (Node = RootNode) or (Node = nil) then
    ...
    {$MESSAGE 'GTA -----'}
    //  node Object keeping another node object
    result:= (Node as TMVCTreeViewNode);
end;
...
function TMVCEditLink.PrepareEdit(Tree:TojBaseVirtualTree;Node:TojVirtualNode;Column:TColumnIndex): Boolean;
