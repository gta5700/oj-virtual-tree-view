...
uses
  ...
  ojVirtualTrees, ojVTAccessibilityFactory, Vcl.Controls;

type
  TVirtualTreeAccessibility = class(TInterfacedObject, IDispatch, IAccessible)
  private
    FVirtualTree: TojVirtualStringTree;
  public
    constructor Create(AVirtualTree: TojVirtualStringTree);
  ...
  public
    function CreateIAccessible(ATree: TojBaseVirtualTree): IAccessible;
  end;
...
//----------------------------------------------------------------------------------------------------------------------
constructor TVirtualTreeAccessibility.Create(AVirtualTree: TojVirtualStringTree);
...
function TVTDefaultAccessibleProvider.CreateIAccessible(ATree: TojBaseVirtualTree): IAccessible;
begin
  result := TVirtualTreeAccessibility.Create(TojVirtualStringTree(ATree));
end;

{ TVTDefaultAccessibleItemProvider }

function TVTDefaultAccessibleItemProvider.CreateIAccessible(ATree: TojBaseVirtualTree): IAccessible;
begin
  result := TVirtualTreeItemAccessibility.Create(TojVirtualStringTree(ATree));
end;

{ TVTMultiColumnAccessibleItemProvider }

function TVTMultiColumnAccessibleItemProvider.CreateIAccessible(ATree: TojBaseVirtualTree): IAccessible;
begin
  result := nil;
  if TojVirtualStringTree(ATree).Header.UseColumns then
    result := TVTMultiColumnItemAccessibility.Create(TojVirtualStringTree(ATree));
end;
