unit ojVirtualTrees.StyleHooks;
...
begin
  if ((Handle = 0) or (DC = 0)) then
    Exit;
  if FHorzScrollBarWindow.Visible and StyleServices.Available and (seBorder in TojBaseVirtualTree(Control).StyleElements) then
  begin
    B := TBitmap.Create;
    try
...
begin
  ...
  if FVertScrollBarWindow.Visible and StyleServices.Available and
    (seBorder in TojBaseVirtualTree(Control).StyleElements) then
  begin
    B := TBitmap.Create;
    try
      ...
      with FVertScrollBarRect do
        BitBlt(DC, Left, Top, B.Width, B.Height - TojBaseVirtualTreeCracker(Control).BorderWidth, B.Canvas.Handle, 0, 0, SRCCOPY);
    finally
      B.Free;
    end;
  end;
...
begin
  Result := TojBaseVirtualTreeCracker(Control).Header.InHeader(P);
end;

procedure TVclStyleScrollBarsHook.UpdateScrollBarWindow;
var
  R: TRect;
  Owner: TojBaseVirtualTree;
  HeaderHeight: Integer;
  BorderWidth: Integer;
begin
  Owner := TojBaseVirtualTree(Control);
  if (hoVisible in TojBaseVirtualTreeCracker(Owner).Header.Options) then
    HeaderHeight := TojBaseVirtualTreeCracker(Owner).Header.Height
  else
    HeaderHeight := 0;
  BorderWidth := 0;
  // VertScrollBarWindow

  if FVertScrollBarWindow.Visible and (seBorder in TojBaseVirtualTree(Control).StyleElements)
  then
  begin
    R := FVertScrollBarRect;
    ...
    SetWindowPos(FVertScrollBarWindow.Handle, HWND_TOP, Control.Left + R.Left +
      TojBaseVirtualTreeCracker(Control).BorderWidth, Control.Top + R.Top + HeaderHeight
      + TojBaseVirtualTreeCracker(Control).BorderWidth, R.Right - R.Left,
      Control.Height - HeaderHeight - BorderWidth - TojBaseVirtualTreeCracker(Control)
  ...
  // HorzScrollBarWindow
  if FHorzScrollBarWindow.Visible and (seBorder in TojBaseVirtualTree(Control).StyleElements)
  then
  begin
    R := FHorzScrollBarRect;
    if Control.BiDiMode = bdRightToLeft then
    ...
    SetWindowPos(FHorzScrollBarWindow.Handle, HWND_TOP, Control.Left + R.Left +
      TojBaseVirtualTreeCracker(Control).BorderWidth, Control.Top + R.Top +
      TojBaseVirtualTreeCracker(Control).BorderWidth + HeaderHeight, R.Right - R.Left,
...
begin
 if (tsWindowCreating in TojBaseVirtualTree(Control).TreeStates) then
  begin
    CalcScrollBarsRect;
    UpdateScrollBarWindow;
 end;
...
begin
  CallDefaultProc(TMessage(Msg));
  if not(tsWindowCreating in TojBaseVirtualTree(Control).TreeStates) then
  begin
    CalcScrollBarsRect;
    UpdateScrollBarWindow;
    PaintScrollBars;
...
initialization
  TCustomStyleEngine.RegisterStyleHook(TojVirtualStringTree, TVclStyleScrollBarsHook);
  TCustomStyleEngine.RegisterStyleHook(TojVirtualDrawTree, TVclStyleScrollBarsHook);

finalization
  TCustomStyleEngine.UnRegisterStyleHook(TojVirtualStringTree, TVclStyleScrollBarsHook);
  TCustomStyleEngine.UnRegisterStyleHook(TojVirtualDrawTree, TVclStyleScrollBarsHook);
