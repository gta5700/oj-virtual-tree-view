...
//
// September 2004:
//  - Bug fix: TojVTHeaderPopupMenu.OnMenuItemClick used the wrong Tag member for the event.
// 
...
type
  ...
  TAddHeaderPopupItemEvent = procedure(const Sender: TojBaseVirtualTree; const Column: TColumnIndex;
    var Cmd: TAddPopupItemType) of object;
  TColumnChangeEvent = procedure(const Sender: TojBaseVirtualTree; const Column: TColumnIndex; Visible: Boolean) of object;

  TVTMenuItem = TMenuItem;

  TojVTHeaderPopupMenu = class(TPopupMenu)
  strict private
...
//----------------- TojVTHeaderPopupMenu ---------------------------------------------------------------------------------

procedure TojVTHeaderPopupMenu.DoAddHeaderPopupItem(const Column: TColumnIndex; out Cmd: TAddPopupItemType);

begin
  Cmd := apNormal;
  if Assigned(FOnAddHeaderPopupItem) then
    FOnAddHeaderPopupItem((PopupComponent as TojBaseVirtualTree), Column, Cmd);
end;

//----------------------------------------------------------------------------------------------------------------------

procedure TojVTHeaderPopupMenu.DoColumnChange(Column: TColumnIndex; Visible: Boolean);

begin
  if Assigned(FOnColumnChange) then
    FOnColumnChange((PopupComponent as TojBaseVirtualTree), Column, Visible);
end;

//----------------------------------------------------------------------------------------------------------------------

procedure TojVTHeaderPopupMenu.OnMenuItemClick(Sender: TObject);

begin
  if Assigned(PopupComponent) and (PopupComponent is TojBaseVirtualTree) then begin
    if TVTMenuItem(Sender).Name = cResizeToFitMenuItemName then begin
      TojBaseVirtualTree(PopupComponent).Header.AutoFitColumns();
    end
    else begin
      with TVTMenuItem(Sender),
        TojBaseVirtualTree(PopupComponent).Header.Columns.Items[Tag] do
      begin
        if Checked then
...
procedure TojVTHeaderPopupMenu.Popup(x, y: Integer);
resourcestring
...
begin
  if Assigned(PopupComponent) and (PopupComponent is TojBaseVirtualTree) then
  begin
    ...
    with (PopupComponent as TojBaseVirtualTree).Header do
    begin
